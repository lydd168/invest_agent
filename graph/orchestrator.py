...
def run_pipeline(user_input: Dict) -> Path:
    ...
    def summary_node(state: PipelineState) -> PipelineState:
        summary_agent.run(state)
        return {"summary_insight": state.get("summary_insight", [])}
    ...
    # 當 join ready 才觸發 summary
    try:
        graph.add_conditional_edges(
            "join_gate",
            _join_ready,
            {"ready": "summary", "wait": "noop"},
        )
    except Exception:
        graph.add_edge("join_gate", "summary")
    graph.add_edge("summary", "reporter")
